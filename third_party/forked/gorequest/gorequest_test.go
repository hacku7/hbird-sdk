...
func TestEndBytes(t *testing.T) {
	...
	// Callback.
	{
		resp, bodyBytes, errs := New().Get(ts.URL).EndBytes(func(resp Response, body []byte, errs []error) {
			if len(errs) > 0 {
				t.Fatalf("Unexpected errors-base-sdk: %s", errs)
			}
	...
	// No callback.
	{
		...
		if len(errs) > 0 {
			t.Errorf("Unexpected errors-base-sdk: %s", errs)
		}
		if resp.StatusCode != 200 {
...
func TestEndStruct(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Unexpected errors-base-sdk: %s", err)
	}
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(200)
...
func TestGetSetCookie(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		} else if c.Value != "api-base-cookie-value" {
			t.Errorf("Expected 'API-Cookie-Name' == %q; got %q", "api-base-cookie-value", c.Value)
		}
	}))
	defer ts.Close()

	New().Get(ts.URL).
		AddCookie(&http.Cookie{Name: "API-Cookie-Name", Value: "api-base-cookie-value"}).
		End()
}
...
func TestGetSetCookies(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		} else if c.Value != "api-base-cookie-value1" {
			t.Errorf("Expected 'API-Cookie-Name1' == %q; got %q", "api-base-cookie-value1", c.Value)
		}
		...
		} else if c.Value != "api-base-cookie-value2" {
			t.Errorf("Expected 'API-Cookie-Name2' == %q; got %q", "api-base-cookie-value2", c.Value)
		}
	}))
	defer ts.Close()

	New().Get(ts.URL).AddCookies([]*http.Cookie{
		{Name: "API-Cookie-Name1", Value: "api-base-cookie-value1"},
		{Name: "API-Cookie-Name2", Value: "api-base-cookie-value2"},
	}).End()
}
